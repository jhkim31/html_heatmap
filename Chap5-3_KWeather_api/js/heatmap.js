import { grid_data_x5_05 as stationData } from './data.js';
...
var HeatMap = function () {
    ...
    var gap = 0.05          //grid size
    var selectRange = 0.3   //interpolation scope
    var grid = []
    ...
    function selectStations(latitude, longitude) 
    {
        ...
        {
            if (!checkRange || (checkCount > 0 && checkCount < 10)) 
            {
                if (stationData[i].la < latitude + selectRange && stationData[i].la >= latitude - selectRange) 
                {
                    if (stationData[i].lo > longitude - selectRange && stationData[i].lo < longitude + selectRange) 
                    {                        
    ...
    function IDWInterpolation(latitude, longitude, stations) {
        ...
        for (var i = 0; i < stations.length; i++) {
            var d = getDistanceFromLatLonInKm(parseFloat(stations[i].la), parseFloat(stations[i].lo), latitude, longitude);
            if (d == NaN) {
                debugger;
                console.log(stations[i])
            }
            sum1 += (stations[i].p10 / (d * d));
            sum2 += (1 / (d * d));
        }
        if (sum2 != 0) {
            return sum1 / sum2;
    ...
    map.on('click', (e) => {
        console.log(e)        
        console.log(getValue(e.containerPoint.x, e.containerPoint.y))
    })
